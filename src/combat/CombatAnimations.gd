...
func targetattack(node, args = null):
	...
	return nextanimationtime + aftereffectdelay + shot
...
const ASSASS_LEAD = 0.95 #уход, перенос и появление - удар в конце
const ASSASS_BACK = 0.70 #возвращение на место
const ASSASS_OFF = 94.0 #насколько зайти за цель
const ASSASS_Y = 26.0 #и насколько ниже неё встать
const ASSASS_DIST_BACK = 22.0 #отступ назад перед уходом в тень
...
func assassinate_step(node, args = null):
	...
	#перекрытие задать нельзя: у Control в Godot 3 нет z_index, порядок только по дереву.
	#Для союзника это и не нужно - Panel2 рисуется после Panel, и вражеская карточка
	#накрывает его сама. У вражеского ассасина будет наоборот, поверх.

	var p = node.rect_position
	var v = node.get_attack_vector().normalized()
	var dest = p
	if args.has('foe_node') and is_instance_valid(args.foe_node):
		#разные контейнеры, поэтому считаем смещение в глобальных координатах
	...
	#перенос идёт вслепую, пока узел прозрачен - выглядит как рывок сквозь тень
	...
	#появление за спиной: доводит клинок обратно к цели
	...
	#замах оружия ставим так, чтобы его собственный кадр удара лёг ровно в L
	var wname = 'at_sword'
	...
	#уход обратно: снова тает, возвращается вслепую и мягко проявляется дома
	var B = ASSASS_BACK
...
#разворачиваем только портрет: зеркалить карточку нельзя, поедут имя и полоски
...
func assassinate(node, args = null):
	...
	#бьют из-за спины, поэтому спрайт зеркалим относительно обычного
...
func caster_cut(node, contact):
	...
	node.rect_scale = Vector2(1,1)
	node.rect_rotation = 0
	node.modulate.a = 1.0 #на случай, если предыдущий теневой шаг не доиграл возврат
	assass_set_facing(node, false)
	var tween = input_handler.GetTweenNode(node)
	var p = node.rect_position
